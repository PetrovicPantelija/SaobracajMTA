...
namespace Saobracaj.Mobile
{
    public partial class frmPrijavaSmene : Form
    {
        ...
        {
            var select = "Select distinct top 200 ZaposleniPrijava.ID,  AktivnostiStavke.OznakaPosla , ZaposleniPrijava.Zaposleni as ZaposleniID, Rtrim(DeIme) + ' ' + Rtrim(DePriimek) as Zaposleni,DatumPrijave, DatumOdjave, " +
...
" inner join AktivnostiStavke on AktivnostiStavke.IDNadredjena = Aktivnosti.ID " + 
        ...
        {
            var select = "Select distinct top " + BrZapisa.Value + "  ZaposleniPrijava.ID,  AktivnostiStavke.OznakaPosla , ZaposleniPrijava.Zaposleni as ZaposleniID, Rtrim(DeIme) + ' ' + Rtrim(DePriimek) as Zaposleni,DatumPrijave, DatumOdjave, " +
                " AktivnostID, LongPrijave, LatPrijave, LongOdjave, LatOdjave,  DateDiff(minute, DatumPrijave,DatumOdjave) as Minuta from ZaposleniPrijava " +
            " inner join Delavci on DeSifra = ZaposleniPrijava.Zaposleni " +
           " inner join Aktivnosti on Aktivnosti.ID = ZaposleniPrijava.AktivnostID " +
" inner join AktivnostiStavke on AktivnostiStavke.IDNadredjena = Aktivnosti.ID " +
        ...
        private void RefreshDataGridPoVremenu()
        {
            string Godina = "";
            string Mesec = "";
            string Dan = "";

            string Vreme = dateTimePicker1.Text;
            char[] spearator = { '.' };
            Int32 count = 3;

            // Using the Method
            String[] strlist = Vreme.Split(spearator,
                   count, StringSplitOptions.None);


            Mesec = strlist[1].ToString();
            Dan = strlist[0].ToString();
            Godina = strlist[2].ToString();
            string FormatiranoVreme = Dan.ToString() + "." + Mesec.ToString() + "." + Godina.ToString();

            var select = "Select  ZaposleniPrijava.ID,  AktivnostiStavke.OznakaPosla , ZaposleniPrijava.Zaposleni as ZaposleniID, Rtrim(DeIme) + ' ' + Rtrim(DePriimek) as Zaposleni,DatumPrijave, DatumOdjave, " +
                " AktivnostID, LongPrijave, LatPrijave, LongOdjave, LatOdjave,  DateDiff(minute, DatumPrijave,DatumOdjave) as Minuta from ZaposleniPrijava " +
            " inner join Delavci on DeSifra = ZaposleniPrijava.Zaposleni " +
           " inner join Aktivnosti on Aktivnosti.ID = ZaposleniPrijava.AktivnostID " +
" inner join AktivnostiStavke on AktivnostiStavke.IDNadredjena = Aktivnosti.ID " +
            " where DatumPrijave  >= " + "Cast('" + FormatiranoVreme.ToString() + "' as DateTime)" + " order by ZaposleniPrijava.ID desc";
