...
using static Saobracaj.Skladista.PrijemnicaCarinskoSkladiste;
...
namespace Saobracaj.Skladista_main.Dokumenta
{
    public partial class Prijemnica : Form
    {
        ...
        private void VratiPodatke(int id)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("select ID From RNCarinskoSkladistePrijemnica Where RN=" + id, conn))
                {
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            txtPrijemnica.Text = dr["ID"].ToString();
                        }
                    }
                }
                conn.Close();
      ...
      ,[Aktivan]
      ,[Formiran]
	  ,RadniNalogSkladista.CarinskiPostupak as CarinskiPostupak
...
Where RadniNalogSkladista.ID=" + id, conn);
        ...
        private void btnIzdajNalog_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    ins.InsertNalogRukovalac(Convert.ToInt32(txtPrijemnica.Text), Convert.ToInt32(cboRukovalac.SelectedValue), row.Cells["Pozicija"].Value.ToString(), Convert.ToDecimal(row.Cells["Koleta"].Value),
