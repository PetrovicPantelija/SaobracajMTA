...
namespace Saobracaj.Nepravilnosti
{
    public partial class frmNeispravnostPostupak : Form
    {
        ...
        private void frmNeispravnostPostupak_Load(object sender, EventArgs e)
        {
            FillGV();
        }
        private void FillGV()
        {
            var select = "Select * From sifNeispravnostPostupak order by ID desc";
        ...
        private void tsSave_Click(object sender, EventArgs e)
        {
            ...
            if (status == true)
            {
                ins.InsSifNeispravnostPostupak(txtNaziv.Text.ToString().TrimEnd(), txtOpis.Text.ToString().TrimEnd());
            }
            else
            {
                ins.UpdSifNeispravnostPostupak(Convert.ToInt32(txtID.Text), txtNaziv.Text.ToString().TrimEnd(), txtOpis.Text.ToString().TrimEnd());
            }
        ...
        {
            Nepravilnosti.InsertNepravilnosti ins = new InsertNepravilnosti();
            ins.DelSifNeispravnostPostupak(Convert.ToInt32(txtID.Text));
            FillGV();
        }
