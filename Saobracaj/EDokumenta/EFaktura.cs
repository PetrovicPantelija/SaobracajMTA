...
{
    public partial class EFaktura : Form
    {
        ...
        static XNamespace xsd = XNamespace.Get("http://www.w3.org/2001/XMLSchema");
        static XNamespace sbt = XNamespace.Get("http://mfin.gov.rs/srbdt/srbdtext");
        ...
        private void EFaktura_Load(object sender, EventArgs e)
        {
            FillCombo();
        }
        private void FillCombo()
        {
            var query = "Select FaStFak as [BR] From Faktura Where FaStatus='PO' and FaVrstDok='RA' order by FaStFak desc";
            ...
            var avans = "Select FaStFak as [BR] From Faktura Where FaStatus='ZA' and FaVrstDok='AV' order by FaStFak desc";
            SqlDataAdapter da2 = new SqlDataAdapter(avans, conn);
            DataSet ds2 = new DataSet();
            da2.Fill(ds2);
            cbo_Avans.DataSource = ds2.Tables[0];
            cbo_Avans.DisplayMember = "BR";
            cbo_Avans.ValueMember = "BR";
        }
        ...
        #region pomPlatilac
        int platilacID;
        string faktura, valuta, primalac, adresa, grad, postanski, punaAdresa, drzava, mb, mail, kreirao, napomena1, napomena2, razlog, modul, pib, pibPom, jbkjs, racun, gln, tip, porudzbenica = "";
        ...
        private void PodaciPlatilac()
        {
            var query = "Select FaStFak as [Broj Racuna],FaVpisalDat as [Datum izdavanja],FaDatVal as [Valuta placanja],FaValutaCene as [Valuta],FaPartPlac as [PrimalacID],"
                        ...
                        + "BankeFak.BFOpomba as Racun,RTrim(PartnerjiDod.PaDEAN) as GLN,partnerjiDod.PaDNasaSif as [JBKJSTip], FaStKupNar as [Porudzbenica]  "
                        + "From Faktura "
            ...
            try
            {
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        ...
                        tip = dr[22].ToString().ToString().TrimEnd();
                        porudzbenica = dr[23].ToString().TrimEnd();
                    }
                }
                else
                {
                    MessageBox.Show("Unesite broj banke!");
        ...
        private void PodaciStavke()
        {
            ...
            conn.Close();
            var queryBrNar = "Select distinct DoStKupNar From DObavnica inner join FakturaPostav on Dobavnica.DOStDob=FakturaPostav.FaPStDokRef Where FakturaPostav.FaPStFak=" + Convert.ToInt32(cbo_Faktura.SelectedValue);
            conn.Open();
        ...
        private void Validacija()
        {
            ...
            if (txt_Racun.Text == "")
            {
                Poruka += "Morate uneti broj računa na koji će se izvršiti uplata! \n";
            }
        ...
        private void btn_Dodatna_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < path.Length; i++)
            {
                foreach (string s in path)
                {
                    ...
                    if (pomTip == ".xlsx") { mime = @"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"; }
                    if (pomTip == ".pdf" || pomTip == ".PDF") { mime = @"application/pdf"; }
                    if (pomTip == ".doc" || pomTip == ".docx") { mime = @"application/msword"; }
                    if (pomTip == ".png") { mime = @"image/png"; }
                    if (pomTip == ".jpeg") { mime = @"image/jpeg"; }
                    if (pomTip == ".csv") { mime = @"text/csv"; }
                    prilozi.Add(
        ...
        #region pomAvans
        XElement xmlAvans = new XElement(cac + "BillingReference");
        string oznakaAvansa = "";
        decimal osnovicaAvans = 0;

        decimal porezAvans = 0;
        int avansPDV = 0;
        decimal iznosAvansa = 0;
        string avansValuta = "";
        DateTime avansDatum;

        string avansModul = "";
        decimal inoPDV = 0;
        XElement avansEXT = new XElement(cec + "UBLExtensions");
        XElement avansTax = new XElement(cac + "TaxTotal");
        XElement avansSub = new XElement(cac + "TaxSubtotal");
        XElement avansIno = new XElement(cbc + Const.Ino_Tax, "RSD");
        XElement avansKat = new XElement(cac + "TaxCategory");
        int konacnaPDV;
        #endregion
        private void Avans()
        {
            var query = "select FaPstFak,(FaPZnesPost+FaPZnesPopusta) as osnovica,FaPZnesDavka as Porez,FaPDavZapSt,FaVpisalDat,FaValutaCene,Faktura.FaModul " +
       "From FakturaPostav " +
       "Inner join Faktura on FakturaPostav.FaPStFak = Faktura.FaStFak " +
       "Where FaPOznaka<>'D' and FaPStFak = " + Convert.ToInt32(cbo_Avans.SelectedValue);
            SqlConnection conn = new SqlConnection(connect);
            SqlCommand cmd = new SqlCommand(query, conn);
            conn.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            try
            {
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        oznakaAvansa = dr[0].ToString().TrimEnd();
                        osnovicaAvans = Convert.ToDecimal(dr[1].ToString());
                        porezAvans = Convert.ToDecimal(dr[2].ToString());
                        iznosAvansa = osnovicaAvans + porezAvans;
                        avansPDV = Convert.ToInt32(dr[3].ToString());
                        avansDatum = Convert.ToDateTime(dr[4].ToString());
                        avansValuta = dr[5].ToString();
                        avansModul = dr[6].ToString().TrimEnd();
                    }
                }
            }
            catch { }
            ...
            if (avansModul == "INO")
            {
                var tecaj = "Select TeVrednost from Tecaj Where TeDatum=" + avansDatum + " and TeValuta='" + avansValuta + "'";
                SqlCommand cmd2 = new SqlCommand(tecaj, conn);
                conn.Open();
                SqlDataReader dr2 = cmd2.ExecuteReader();
                try
                {
                    if (dr2.HasRows)
                    {
                        while (dr2.Read())
                        {
                            inoPDV = porezAvans * Convert.ToDecimal(dr2[0].ToString());
                        }
                    }
                    else
                    {
                        MessageBox.Show("Za zadati avansni račun nije upisan kurs valute");
                        return;
                    }
                }
                catch { }
            ...
            xmlAvans.Add(
                ...
                new XElement(cbc + "ID", oznakaAvansa) //oznakaAvansa
                , new XElement(cbc + "IssueDate", datumIzdavanja.ToString("yyyy-MM-dd"))
                ));

            if (avansPDV == 1 || avansPDV == 21 || avansPDV == 50 || avansPDV == 23)
            {
                avansSub.Add(
                new XElement(cbc + Const.Osnovica, osnovicaAvans.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                new XElement(cbc + Const.UkupnoPorez, porezAvans.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                new XElement(cac + Const.Kategorija,
                new XElement(cbc + Const.ID, "S"),
                new XElement(cbc + Const.Procenat, "20"),
                new XElement(cac + Const.TaxScheme,
                ...
                avansTax.Add(new XElement(cbc + Const.UkupnoPorez, porezAvans.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)), avansSub);

                if (avansModul == "INO")
                {
                    avansIno.Add(new XElement(cbc + Const.Porez, inoPDV.ToString("F").Replace(",", "."),
                        new XAttribute("currencyID", "RSD")));
                }
            ...
            if (avansPDV == 38)
            {
                avansSub = new XElement(cac + Const.TaxSub);
                avansSub.Add(
                new XElement(cbc + Const.Osnovica, osnovicaAvans.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                new XElement(cbc + Const.UkupnoPorez, porezAvans.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                new XElement(cac + Const.Kategorija,
            ...
            var queryD = "select FaPsifra From FakturaPostav Where FaPOznaka='D' And FaPStFak=" + Convert.ToInt32(cbo_Faktura.SelectedValue);
            ...
            try
            {
                if (drD.HasRows)
                {
                    while (drD.Read())
                    {
                        konacnaPDV = Convert.ToInt32(drD[0].ToString());
                    }
                }
            }
            catch { }
            ...
            decimal ukupnoPorez = Convert.ToDecimal(txt_p20.Text) + Convert.ToDecimal(txt_p10.Text) + Convert.ToDecimal(txt_p0.Text);
            decimal ukupnoOsnovica = Convert.ToDecimal(txt_o20.Text) + Convert.ToDecimal(txt_o10.Text) + Convert.ToDecimal(txt_o0.Text);
            decimal preostalaOsnovica = ukupnoOsnovica - osnovicaAvans;
            decimal preostaliPorez = 0;

            if (konacnaPDV == 1 || konacnaPDV == 21 || konacnaPDV == 50 || konacnaPDV == 52 || konacnaPDV == 23)
            {
                preostaliPorez = preostalaOsnovica * 20 / 100;
                avansKat.Add(
                    new XElement(cbc + "ID", "S"),
                    new XElement(cbc + "Percent", "20"),
                    new XElement(cac + "TaxScheme",
                    new XElement(cbc + "ID", "VAT")));
            }
            if (konacnaPDV == 2 || konacnaPDV == 38 || konacnaPDV == 51 || konacnaPDV == 53)
            {
                preostaliPorez = preostalaOsnovica * 20 / 100;
                avansKat.Add(
                    new XElement(cbc + "ID", "S"),
                    new XElement(cbc + "Percent", "10"),
                    new XElement(cac + "TaxScheme",
                    new XElement(cbc + "ID", "VAT")));
            }
            if (konacnaPDV == 56 || konacnaPDV == 62)
            {
                preostaliPorez = 0;
                avansKat.Add(
            ...
            if (konacnaPDV == 57 || konacnaPDV == 61)
            {
                preostaliPorez = 0;
                avansKat.Add(
                    new XElement(cbc + "ID", "O"),
                    new XElement(cbc + "Percent", "0"),
                    new XElement(cac + "TaxScheme",
                    new XElement(cbc + "ID", "VAT")));
            }
            ...
            avansEXT.Add(
                 ...
                 new XElement(xsd + "InvoicedPrepaymentAmmount",
                 new XElement(cbc + "ID", oznakaAvansa), //oznakaAvansa
                 avansTax),
                 new XElement(xsd + "ReducedTotals",
                 new XElement(cac + "TaxTotal",
                 new XElement(cbc + "TaxAmount", ukupnoPorez.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                 new XElement(cac + "TaxSubtotal",
                 new XElement(cbc + "TaxableAmount", preostalaOsnovica.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                 new XElement(cbc + "TaxAmount", preostaliPorez.ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                 avansKat)),
                 new XElement(cac + "LegalMonetaryTotal",
                 new XElement(cbc + "TaxExclusiveAmount", (ukupnoOsnovica - iznosAvansa).ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                 new XElement(cbc + "TaxInclusiveAmount", ((ukupnoOsnovica - iznosAvansa) + preostaliPorez).ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)),
                 new XElement(cbc + "PayableAmount", ((ukupnoOsnovica - iznosAvansa) + preostaliPorez).ToString("F").Replace(",", "."), new XAttribute("currencyID", avansValuta)))))))
                 );
        }
        private void btn_Avans_Click(object sender, EventArgs e)
        {
            Avans();
            decimal zaPlacanje = Convert.ToDecimal(textBox7.Text) - iznosAvansa;
            textBox7.Text = zaPlacanje.ToString();
        }
        ...
        private void CreateXML()
        {
            ...
            using (XmlWriter xw = XmlWriter.Create(filename, xws))
            {
                if (iznosAvansa != 0)
                {
                    el = new XElement(xmlns + Const.Invoice,
                     new XAttribute("xmlns", xmlns), new XAttribute(XNamespace.Xmlns + "xsi", xsi),
                     new XAttribute(XNamespace.Xmlns + "xsd", xsd),
                     new XAttribute(XNamespace.Xmlns + "sbt", sbt),
                ...
                el.Add(
                    ...
                    new XElement(cbc + Const.OznakaDok, "380"),
                    ...
                    new XElement(cbc + Const.DescriptionCode, 35)),
                    new XElement(cac + Const.Narudzbenica,
                    new XElement(cbc + Const.ID, porudzbenica)),
                    new XElement(cac + "DespatchDocumentReference",
                    new XElement(cbc + "ID", nizBrNar)),
                    xmlAvans,
                    //PDF
                    ...
                    new XElement(cbc + Const.Avans, iznosAvansa.ToString("F").Replace(",", "."), new XAttribute("currencyID", valuta)),
                    new XElement(cbc + Const.Ukupno, ((ukupnoO + ukupnoP) - iznosAvansa).ToString("F").Replace(",", "."), new XAttribute("currencyID", valuta))));
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    ...
                    if (rabat != 0)
                    {
                        ...
                        iznosRabata = Convert.ToDecimal((osnovica * rabat) / 100);
                        novaCena = osnovica - iznosRabata;
                        if (pdvPom == 1 || pdvPom == 21 || pdvPom == 50 || pdvPom == 23)
                        {
                ...
                if (xmlAvans.IsEmpty == true)
                {
                    xmlAvans.Remove();
                    avansEXT.Remove();
                }
                /*if (avansEXT.IsEmpty == true)
                {
                    avansEXT.Remove();
                }*/
        ...
        private void knjižnaZaduženjaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            EZaduzenje ez = new EZaduzenje();
            ez.Show();
        }
