...
{
    public partial class EOdobrenje : Form
    {
        public string connect = frmLogovanje.connectionString;
        static XNamespace xmlns = XNamespace.Get("urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2");
        ...
        private void EOdobrenje_Load(object sender, EventArgs e)
        {
            FillCombo();
        }
        private void FillCombo()
        {
            var query = "Select FaStFak as [BR] From Faktura Where FaStatus='PO' and FaVrstDok='DO' order by FaStFak desc";
        ...
        private void Primalac()
        {
            xmlPrimalac.Add(
                ...
                new XElement(cbc + Const.Mail, txt_Mail.Text))));
        ...
        private void CreateXML()
        {
            ...
            {
                XElement el = new XElement(xmlns + Const.CreditNote,
                     new XAttribute("xmlns", xmlns), new XAttribute(XNamespace.Xmlns + "xsi", xsi), new XAttribute(XNamespace.Xmlns + "xsd", xsd), new XAttribute(XNamespace.Xmlns + "cac", cac),
                ...
                el.Add(
                    ...
                    new XElement(cbc + Const.CreditTypeCode, "381"),
                    ...
                    new XElement(cbc + Const.DescriptionCode, 0)),
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    ...
                    if (rabat != 0)
                    {
                        ...
                        if (pdvPom == 1 || pdvPom == 21 || pdvPom == 50)
                        {
                            var il = new XElement(cac + Const.OdobrenjeStavke);
                            il.Add(
                                new XElement(cbc + Const.ID, row.Cells[0].Value.ToString()),
                                   new XElement(cbc + Const.OdobrenjeKolicina, row.Cells[3].Value.ToString().Replace(",", "."), new XAttribute("unitCode", jm)),
