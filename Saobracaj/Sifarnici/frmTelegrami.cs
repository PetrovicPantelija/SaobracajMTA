...
namespace Saobracaj.Sifarnici
{
    public partial class frmTelegrami : Form
    {
        ...
        {
            var query = "Select Telegrami.ID,BrojTelegrama,RTrim(Pruga.Opis) as [Naziv]," +
                "RTrim(Stanice.Opis) as [Stanica OD],Rtrim(s.Opis) as [Stanica DO],RTrim(Kolosek) as Kolosek,VaziOD,VaziDo,TrajeOd,TrajeDO,Aktivan,Napomena,PDF,NarocitaPosiljka " +
                "From telegrami " +
                "Inner Join Pruga on Telegrami.PrugaID = Pruga.ID " +
                "Inner join Stanice on Telegrami.OdStanice = Stanice.ID " +
                "Inner join Stanice as s on Telegrami.DoStanice = s.ID " +
            ...
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Selected)
                    {
                        ...
                        dt_TrajeOd.Value = Convert.ToDateTime(row.Cells[8].Value.ToString());
                        dt_TrajeDo.Value = Convert.ToDateTime(row.Cells[9].Value.ToString());
                        bool aktivan, narocita;
                        aktivan = Convert.ToBoolean(row.Cells[10].Value);
                        if (aktivan == true) { cb_Aktivni.Checked = true; } else { cb_Aktivni.Checked = false; }
                        txt_Napomena.Text = row.Cells[11].Value.ToString();
                        txt_PDF.Text = row.Cells[12].Value.ToString();
                        narocita = Convert.ToBoolean(row.Cells[13].Value.ToString());
